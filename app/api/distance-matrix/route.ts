...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingDestinationIds = new Set(existingDistances.map((d: unknown) => (d as { destinationId: string }).destinationId))
    const newDestinationIds = destinationIds.filter(id => !existingDestinationIds.has(id))

    const results: DistanceMatrixResult[] = []

    // Add existing distances to results
    existingDistances.forEach((distance: unknown) => {
      results.push({
        propertyId: (distance as { propertyId: string }).propertyId,
        destinationId: (distance as { destinationId: string }).destinationId,
        drivingDistance: (distance as { drivingDistance?: number }).drivingDistance || undefined,
        drivingDuration: (distance as { drivingDuration?: number }).drivingDuration || undefined,
        trafficDuration: (distance as { trafficDuration?: number }).trafficDuration || undefined,
        status: 'OK'
      })
    })
    ...
    if (newDestinationIds.length > 0) {
      const newDestinations = destinations.filter((d: unknown) => newDestinationIds.includes((d as { id: string }).id))
      ...
      if ((data as { status: string }).status === 'OK') {
        ...
        for (let i = 0; i < ((data as { rows: unknown[] }).rows[0] as { elements: unknown[] }).elements.length; i++) {
          const element = ((data as { rows: unknown[] }).rows[0] as { elements: unknown[] }).elements[i]
          const destination = newDestinations[i]
          
          if ((element as { status: string }).status === 'OK') {
            const distance = (element as { distance: { value: number } }).distance.value // in meters
            const duration = (element as { duration: { value: number } }).duration.value // in seconds
            const trafficDuration = (element as { duration_in_traffic?: { value: number } }).duration_in_traffic?.value // in seconds
